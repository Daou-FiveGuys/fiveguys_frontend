 ...
 * @returns
 */
export default function AddressBook({ onSelectContacts }: AddressBookProps) {
  ...
  const [currentGroup2, setCurrentGroup2] = useState<Group2>(
    sampleData[0].group2s[0]
  ) // 초기값은 가장 빠른 값.
      ...
      folder2.group2s.forEach(group2 => {
        ...
        const matchedContacts = group2.contact2s.filter(contact2 => {
          ...
          if (searchFilter === 'name' || searchFilter === 'both') {
            if (contact2.name.toLowerCase().includes(searchTerm.toLowerCase()))
              return true
          }
  ...
  const deleteAddress = async (contact2Id: number) => {
    ...
    try {
      ...
      const updatedFolders = updateFolderRecursively(
        ...
        group2 => ({
          ...group2,
          contact2s: group2.contact2s.filter(
            ct2 => ct2.contactId !== contact2Id
          )
        })
      )
    ...
    <div>
      {searchResults.map((result: any, index: number) => (
        <div key={result.folder?.folderId || index} className="mb-6">
          <h3 className="text-lg font-semibold mb-2">{result.group2.name}</h3>
          <AddressListView
