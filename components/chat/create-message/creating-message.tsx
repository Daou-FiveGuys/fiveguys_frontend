...
const CreatingMessage: React.FC<CreatingMessageProps> = ({
  buttonType,
  lastUserInput,
  setActiveButton
}) => {
    ...
    switch (stage) {
      ...
      case 'directInput':
        ...
        ChatUtils.addChat(
          buttonType,
          'assistant',
          '입력하신 내용이 저장되었습니다. 이미지 옵션을 선택해주세요: "이미지 생성" 또는 "미생성"'
        )
        break
      case 'autoGenerate':
      ...
      case 'imageOption':
        if (input === '이미지 생성') {
          ...
          ChatUtils.addChat(
            buttonType,
            'assistant',
            '이미지를 추가하시겠습니까? "이미지 추가" 또는 "이미지 없이"를 선택해주세요.'
          )
        } else {
          ChatUtils.addChat(buttonType, 'user', input)
          ChatUtils.addChat(
      ...
      case 'imageSubOption':
        if (input === '이미지 추가') {
          ...
          ChatUtils.addChat(
            buttonType,
            'assistant',
            '올바른 옵션을 선택해주세요: "이미지 추가" 또는 "이미지 없이"'
          )
        }
        break
    }
