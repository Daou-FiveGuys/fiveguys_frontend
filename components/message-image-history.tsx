...
export const sampleData: HistoryItem[] = [
  {
    id: 1,
...
export default function MessageImageHistory({ id }: { id: number }) {
  const items = id === -1 ? sampleData : [getHistoryItem(id)].filter(Boolean) as HistoryItem[]
  ...
  return (
    <div className="space-y-4">
      {items.map(item => (
        <Card key={item.id} className="w-full max-w-3xl">
          <CardHeader>
            <CardTitle>메시지 및 이미지 히스토리 (고유번호: {item.id})</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
